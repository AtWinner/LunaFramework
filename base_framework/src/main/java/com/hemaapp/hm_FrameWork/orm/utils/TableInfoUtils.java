	...
	public static <T> TableInfo newTable(String dbName, SQLiteDatabase db, Class<T> clazz) {
		...
		try {
			...
			if (cursor != null && cursor.moveToNext()) {
				...
				if (count > 0) {
					cursor.close();
					HemaLogger.d(TAG, "表 %s 已存在"+tableInfo.getTableName());
					...
					for (String newField : newFieldList) {
						...
						HemaLogger.d(TAG, "表"+tableInfo.getTableName()+"新增字段"+newField);
					}
			...
			HemaLogger.d(TAG, "创建一张新表"+tableInfo.getTableName());
		} catch (Exception e) {
			e.printStackTrace();

			HemaLogger.d(TAG, e.getMessage() + "");
		} finally {
			if (cursor != null)
				cursor.close();
			cursor = null;
